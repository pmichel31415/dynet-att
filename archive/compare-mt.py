...
parser.add_argument('--printsize', type=int, default=50,
                    help='How many n-grams to print.')
...
if args.otherout == None:
    ...
    for k, v in scorelist[:args.printsize]:
        print('%s\t%f (ref=%d, out=%d)' %
              (' '.join(k), v, refall[k], outall[k]))
    print()
    print('--- %d under-generated n-grams indicative of reference' %
          args.printsize)
...
# Analyze the differences between two systems
    ...
    scorelist = sorted(scores.items(), key=operator.itemgetter(1))
    # Print the ouput
    print('--- %d n-grams that System 1 did a better job of producing' %
          args.printsize)
    for k, v in scorelist[:args.printsize]:
        print('%s\t%f (sys1=%d, sys2=%d)' %
              (' '.join(k), v, outall[k], out2all[k]))
    print()
    print('--- %d n-grams that System 2 did a better job of producing' %
          args.printsize)
    for k, v in reversed(scorelist[-args.printsize:]):
        print('%s\t%f (sys1=%d, sys2=%d)' %
              (' '.join(k), v, outall[k], out2all[k]))
